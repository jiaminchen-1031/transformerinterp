...
from ViT_new import vit_base_patch16_224 as vit
...
def compute_saliency_and_save(args):
    ...
    #             Add data
            ...
            elif args.method == 'partial_lrp':
                Res = lrp.generate_LRP(data, method="last_layer", is_ablation=args.is_ablation, index=index) \
                        .reshape(data.shape[0], 1, 14, 14)

            elif args.method == 'attn_last_layer':
                Res = baselines.generate_attn(data).reshape(data.shape[0], 1, 14, 14)

            elif args.method == 'attn_gradcam':
                Res = baselines.generate_cam_attn(data, index=index).reshape(data.shape[0], 1, 14, 14)

            elif args.method == 'generic_attribution':
                Res = baselines.generate_genattr(data, index=index, start_layer=2).reshape(data.shape[0], 1, 14, 14)

            elif args.method == 'ours':
                Res = baselines.generate_ours(data, index=index, steps=args.steps, start_layer=4).reshape(data.shape[0], 1, 14, 14)

            elif args.method == 'ours_c':
                Res = baselines.generate_ours_c(data, index=index, steps=args.steps, start_layer=7).reshape(data.shape[0], 1, 14, 14)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method', type=str,
                        ...
                        choices=['rollout', 'partial_lrp', 'transformer_attribution', 
                                 'attn_last_layer', 'attn_gradcam', 'generic_attribution', 'ours', 'ours_c'],
                        help='')
    ...
    if args.vis_class == 'index':
        os.makedirs(os.path.join(PATH, 'visualizations/{}/{}_{}_{}'.format(args.method,
                                                                        args.vis_class,
                                                                        args.class_id, args.steps)), exist_ok=True)
        args.method_dir = os.path.join(PATH, 'visualizations/{}/{}_{}_{}'.format(args.method,
                                                                              args.vis_class,
        ...
        os.makedirs(os.path.join(PATH, 'visualizations/{}/{}/{}_{}'.format(args.method,
                                                                     args.vis_class, ablation_fold, args.steps)), exist_ok=True)
...
#     import random
#     random.seed(0)
#     index = random.sample(range(len(imagenet_ds)), num_samples)
    index = np.load('../pert_index.npy')
    sub_dataset = torch.utils.data.Subset(imagenet_ds, indices=index)

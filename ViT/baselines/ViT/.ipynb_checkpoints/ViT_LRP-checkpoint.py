...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, mlp_head=False, drop_rate=0., attn_drop_rate=0., mae=False, dino=False):
        super().__init__()
        self.mae = mae
        self.dino = dino
        ...
        if mlp_head:
            ...
            self.head = Linear(embed_dim*2, num_classes)
        else:
    ...
    def forward(self, x):
        ...
        if self.mae:
            ...
            x = self.norm(x)
            x =  torch.cat((self.pool1(x, dim=1, indices=torch.tensor(0, device=x.device)).squeeze(1).unsqueeze(-1), self.pool2(x, dim=1, indices=torch.tensor(np.arange(1,x.shape[1]), device=x.device)).mean(1).unsqueeze(-1)), dim=-1)
...
def vit_mae_patch16_224(pretrained=False, **kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, mae=True, **kwargs)
...
def vit_base_patch16_224_dino(pretrained=False, **kwargs):
    model = VisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, dino=True, **kwargs)

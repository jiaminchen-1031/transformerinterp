...
from ViT_new import vit_base_patch16_224_dino
...
# Model
model = vit_base_patch16_224_dino(pretrained=True).cuda()
...
#             .reshape(batch_size, 1, 14, 14)
    ...
    # segmentation test for the GradCam baseline (last attn layer)
        ...
        Res = baselines.generate_attn(image.cuda()).reshape(batch_size, 1, 14, 14)
    
    elif args.method == 'ours':
        Res = baselines.generate_ours(image.cuda(), dino=True, start_layer=args.start_layer).reshape(batch_size, 1, 14, 14)
    
    elif args.method == 'ours_c':
        Res = baselines.generate_ours_c(image.cuda(), dino=True, start_layer=args.start_layer).reshape(batch_size, 1, 14, 14)
        
    elif args.method == 'generic_attribution':
        Res = baselines.generate_genattr(image.cuda()).reshape(batch_size, 1, 14, 14)

...
def main():
    ...
    * decode first the evidence, then run classification for each split
    ...
    if os.path.exists(epoch_save_file):
        ...
        data = test+train+val
        pertub_step = np.arange(0.52,0.68,0.02)
        pertub_acc = [0]*len(pertub_step)
        iterator = tqdm(range(0, len(data), test_batch_size))
        

        for batch_start in iterator:
            batch_elements = data[batch_start:min(batch_start + test_batch_size, len(data))]
            targets = [evidence_classes[s.classification] for s in batch_elements]
            targets = torch.tensor(targets, dtype=torch.long, device=device)
            samples_encoding = [interned_documents[extract_docid_from_dataset_element(s)] for s in batch_elements]
            input_ids = torch.stack(
            ...
            for s in batch_elements:
                ...
                for step_idx, step in enumerate(pertub_step):
                    num_token_perb = int(len(text)*step)
#                     cam = scores_per_word_from_scores_per_token(inp, tokenizer,input_ids[0], cam_target)
                ...
                curr_pert_result = [round(res / (batch_start+1) * 100, 2) for res in pertub_acc]
